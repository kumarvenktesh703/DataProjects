...
# Title Declaration
st.title(':red[Youtube]_DataHarvesting_:chart_with_upwards_trend::bar_chart:')
...
with col1:
    ...
    st.write('''ENTER BELOW :blue[BUTTON] TO STORE :blue[DATA]:boom::boom:''')
    if st.button("__:red[ENTER] TO :red[EXTRACT] AND :red[STORE DATA]__"):
        # YouTube API Access
            ...
            try:
                try:
                    ...
                    if 'items' not in channel_response:
                        ...
                        st.error("ENTER CORRECT CHANNEL ID **channel_id**")
                        return None
                    return channel_response
                except HttpError as e:
                    st.error('SERVER ERROR:heavy_exclamation_mark: CHECK YOUR INTERNET CONNECTION AND TRY AGAIN AFTER SOME TIME',icon='🚨')
                    st.write('An error occurred: %s' % e)
                    return None
            except:
                st.write('YOU HAVE EXCEEDED YOUR YOUTUBE API QUOTA. PLEASE TRY AGAIN TOMORROW:heavy_exclamation_mark:',icon='🚨')
        #Call to function get_channel_data() to extracting the channel data
        ...
        #Function to get videos ids of respective channel
        def get_video_ids(youtube, channel_playlist_id):
            ...
            while True:
                ...
                if not next_page_token:
                    break
            return video_id
        # Call to function get_video_ids()  to extract video ids of respective channel
        video_ids = get_video_ids(youtube, channel_playlist_id)
        #Function to extract videos comments of the videos of respective channel
        def get_video_comments(youtube, video_id, max_comments):
            request = youtube.commentThreads().list(part='snippet', maxResults=max_comments, textFormat="plainText",
                                                    videoId=video_id)
            response = request.execute()
            return response
        # Function to convert the duration of videos in proper format
        ...
        #Function to extract the video data of the respective channel
            ...
            try:
                for video_id in video_ids:
                    ...
                    if duration != 'Not Available':
                        duration = convert_duration(duration)          # Call to function convert_duration() to convert video duration in proper format
                    video['contentDetails']['duration'] = duration
                    video_data.append(video)
            except:
                st.write('You have exceeded your YouTube API quota. Please try again tomorrow.')
            return video_data
        #Call to function get_video_data() to extract the video data of the videos of respective channel
        ...
        for i, video in enumerate(video_data):
            ...
            if video['comment_threads'] is not None:
                ...
                for index, comment_thread in enumerate(video['comment_threads']['items']):
                    ...
                    comments[f"Comment_Id_{index + 1}"] = {'Comment_Id': comment_id, 'Comment_Text': comment_text,
        ...
        st.write(f"DATA UPLOADED.: {upload.upserted_id if upload.upserted_id else upload.modified_count}")

        # Close the connection
        client.close()
with col2:
    st.header(':red[DATA] TRANSFER :red[ZONE!!]')
    ca = certifi.where()
    ...
    st.write('''**CLICK BELOW TO TRANSFER DATA FROM MONGODB TO MYSQL DATABASE.**''')
    if st.button('**:red[TRANSFER] TO :red[MYSQL] DATABASE**'):
        # Retrieve the document with the specified name
        result = collection.find_one({"Channel_Name": document_name})
        client.close()
        # conversion of Channel data json to dataframe
        ...
        # conversion of comment data json to dataframe
...
#Analysis of the channel data
st.header(':red[Channel] Data :red[Analysis] Zone')

# Check available channel data
Check_channel = st.checkbox('**:red[SELECT] CHANNEL :red[FOR] ANALYSIS!!**')
...
if Check_channel:
    ...
    # Storing the channel names to the list
...
#Questions given for Analysis
st.subheader(':red[CHANNELS] ANALYSIS')
...
# Close of sql database connection
